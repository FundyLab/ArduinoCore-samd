...
FILENAME=fundylab_samd_${VERSION}.tar.bz2

rm -f $FILENAME

# Change name in platform.txt
#sed -i "s/name=.*/name=SAMD Hourly Build ${BUILD_NUMBER} (${CURR_TIME})/" platform.txt
sed -i "s/name=.*/name=Fundylab SAMD Boards/" platform.txt
sed -i "s/version=.*/version=${VERSION}/" platform.txt

cd ..
tar --transform="s|$FOLDERNAME|${VERSION}|g"  --exclude=extras/** --exclude=.git* --exclude=.idea -cjf $FILENAME $FOLDERNAME
...
cat extras/package_index.json.FundyLab.template |
#sed "s/%%BUILD_NUMBER%%/${BUILD_NUMBER}/" |
#sed "s/%%CURR_TIME%%/${CURR_TIME_SED}/" |
sed "s/%%VERSION%%/${VERSION}/" |
sed "s/%%FILENAME%%/${FILENAME}/" |
sed "s/%%CHECKSUM%%/${CHKSUM}/" |
sed "s/%%SIZE%%/${SIZE}/" > package_fundylab_index.json
